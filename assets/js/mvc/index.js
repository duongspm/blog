...
class IndexView {
  ...
  renderProjects(projects) {
    ...
    if (!projects.length) {
      this.projectList.innerHTML = "<div style='opacity:.6'>Ch�a c� d? �n.</div>";
  ...
  renderPosts(posts, favoriteIds) {
    ...
    if (!posts.length) {
      this.blogMasonry.innerHTML = `<p style="grid-column:1/-1; text-align:center; opacity:.6">Ch�a c� b�i vi?t n�o.</p>`;
      return;
    }
        ...
        return `
                <article class="blog-item reveal">
                    <div class="glass-card">
                        <div class="post-meta"><span class="category">${post.category || "Kh�c"}</span><time>${post.createdAt ? new Date(post.createdAt.seconds * 1000).toLocaleDateString("vi-VN") : ""}</time></div>
                        <h3>${post.title}</h3>
                        ...
                        <div class="post-actions" style="display:flex; gap:.4rem; margin-top:.5rem; align-items:center;">
                            <button class="magnetic-btn btn-read" data-id="${post.id}"><span class="btn-text">�?c th�m ?</span></button>
                            <button class="magnetic-btn btn-fav" data-id="${post.id}">${isFav ? "? �? y�u th�ch" : "? Y�u th�ch"}</button>
                        </div>
                    </div>
                </article>
            `;
...
class IndexController {
  ...
  bindEvents() {
    ...
    this.view.feedbackForm?.addEventListener("submit", async (e) => {
      e.preventDefault();
      const newFb = {
        name: this.view.feedbackName.value.trim() || "Ng�?i d�ng",
        role: this.view.feedbackRole.value.trim() || "Kh�ch h�ng",
  ...
  setupTypewriter() {
    const words = ["Web Performance", "UI/UX", "Full-stack", "S�ng t?o"];
  ...
  setupWheel() {
    ...
    const draw = () => {
      ...
      if (!items.length) {
        ctx.fillStyle = "#888";
        ctx.fillText("Nh?p �t nh?t 2 m?c", 20, center);
        return;
      }
    ...
    wheelBtn.addEventListener("click", () => {
      ...
      const items = wheelInput.value
        ...
        .map((t) => t.trim())
        .filter(Boolean);
      if (items.length < 2) return alert("Nh?p �t nh?t 2 m?c.");
      spinning = true;
